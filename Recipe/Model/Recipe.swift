...
class Recipe: Identifiable {
    ...
    var name: String
    @Relationship(deleteRule: .cascade) var instructions: [Instruction] = [] // Cascades deletion
    @Relationship(deleteRule: .cascade) var ingredients: [Ingredient] = []  // Cascades deletion
    ...
    init(name: String, instructions: [Instruction] = [], ingredients: [Ingredient] = [], categories: [Category] = [], servings: Int = 0, time: Int = 0, author: String = "", dateAdded: Date = .init(), favorite: Bool = false, notes: String = "") {
        self.id = UUID()
        self.name = name
...
class Category: Identifiable {
    @Attribute(.unique) var id: UUID
    var name: String
    @Relationship(deleteRule: .nullify) var recipes: [Recipe] = [] // Nullifies relationship, does not delete
